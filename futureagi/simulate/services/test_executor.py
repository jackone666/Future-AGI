...
# 注意：run_eval_summary_task 延迟导入，避免循环依赖。
...
class TestExecutor:
    """已废弃：遗留测试执行器。不要在这里添加新任务。

    请改用 Temporal workflows/activities；相关模式可参考 tfc/temporal/。
    ...
    def __init__(
        ...
        self.active_tests: dict[str, dict] = {}  # 跟踪活跃的测试执行。

    def _background_sound_enabled(self, raw_value: Any) -> bool:
        """将 persona 的 background_sound 标记规范化为严格布尔值。"""
    ...
    def monitor_test_execution(self, test_execution, poll_interval=20):
        """
        后台线程：监控并更新某次测试中的 call execution。
        """
        try:
            # 优化查询：限制字段，并只使用必要的 select_related。
            call_executions = (
            ...
            for call in call_executions:
                ...
                # 如果 call 已经处于终态（包括 CANCELLED），则跳过。
                if call.status in [
                ...
                if (
                    ...
                    try:
                        ...
                        if is_outbound:
                            # outbound 呼叫使用用户凭据。
                            # 使用已预加载的 test_execution，避免 N+1 查询。
