"""
ClickHouse Schema 定义：PeerDB CDC + 分析层。

架构概览
=====================
Layer 1  -  CDC 落地表（包括 prompt 表）
            PeerDB 将 PostgreSQL 行镜像到 ReplacingMergeTree 表。
            每张表都有三个 PeerDB 元列，用于支持 exactly-once、保序 CDC。

Layer 2  -  trace_dict dictionary + 反规范化 ``spans`` 表
            ClickHouse Dictionary 将最新 trace 元数据保存在内存中，便于低成本查询。
            Materialized View 会为每个新的 observation span 补充 trace 上下文，
            并将 JSON 属性包拆成类型化 Map 列，用于属性分析。

Layer 3  -  小时级预聚合 rollup
            AggregatingMergeTree 表及配套 MV 提供亚秒级 dashboard 查询
            （span metrics、eval metrics）。

创建顺序
--------------
1. CDC 落地表  (tracer_observation_span, tracer_trace,
   trace_session, tracer_eval_logger)
2. trace_dict dictionary
3. spans（反规范化宽表）
4. spans_mv（写入 ``spans`` 的 MV）
...
# 解析配置中的 CH 数据库名，供 DDL 模板使用。
# 如果未配置，则回退到生产默认值 "futureagi"。
...
def _to_single_node_engine(ddl: str) -> str:
    """将 Replicated*MergeTree 引擎转换为单节点等价引擎。

    OSS/自托管部署通常只运行单个 ClickHouse 节点，没有 shard/replica macros。
    Replicated 引擎会因此失败，报错类似 "No macro 'shard' in config ..."。
    """
...
# ============================================================================
# LAYER 1 - PeerDB CDC 落地表。
# ============================================================================
# 所有表都使用 ReplacingMergeTree(_peerdb_version)，便于 PeerDB
# 重放并去重变更事件。每张表都会附加三个元列：
#   _peerdb_synced_at  DateTime64(6)  - 行落地时的真实时间
#   _peerdb_is_deleted UInt8          - 软删除标记（1 = deleted）
#   _peerdb_version    Int64          - RBMT 去重用的单调版本号
# ============================================================================

# ---------------------------------------------------------------------------
# 1. tracer_observation_span
#    镜像 PostgreSQL 模型 ObservationSpan。
# ---------------------------------------------------------------------------
CDC_OBSERVATION_SPAN = """
CREATE TABLE IF NOT EXISTS tracer_observation_span (
    -- 主键（PG 中是 CharField max_length=255）
    id String,
    ...
    -- 内容字段（PG 中为 JSON，这里存为字符串）
    input String DEFAULT '',
    output String DEFAULT '',

    -- 属性字段（通过 PeerDB 从 JSONB 转为 String）
