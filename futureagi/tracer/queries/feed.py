"""
Error Feed API 的数据库辅助函数。

返回 tracer.types.feed_types 中定义的类型化 dataclass，不返回裸 dict。
该文件是纯数据访问层：不处理 HTTP，也不放业务逻辑，由 service 层组合使用。
"""
...
# 在 SQL 中将 EvalLogger 行规范化为 0..1 分数：
# 优先使用 output_float，否则将 output_bool 转成 1.0/0.0。
# 这与 tracer/views/trace.py 中的模式保持一致，使 eval 聚合只有一种规范形态。
...
def _base_qs(project_ids: List[str]) -> QuerySet:
    ...
    # 排除改版前遗留行（旧 agent-compass）。这些行早于 feed 字段，
    # 没有 issue_group，前端会回退渲染为 K-ID。
    return (
...
# ---------------------------------------------------------------------------
# 批量辅助函数：一次查询处理多个 cluster ID。
# ---------------------------------------------------------------------------


def _fetch_trends_batch(cluster_ids: List[str], days: int = 14) -> dict:
    """
    返回 {cluster_id: [TrendPoint, ...]}，按 `days` 范围内的天级 bucket 聚合。

    bucket 来自 ErrorClusterTraces.created_at 的按天分组。
    """
    if not cluster_ids:
        return {}
    ...
    for row in rows:
        ...
        if cid in result:
            bucket = row["bucket"]
            # TruncDate 返回 date，但 serializer 需要 datetime。
...
def _fetch_users_affected_batch(cluster_ids: List[str]) -> dict:
    """
    返回 {cluster_id: distinct_end_user_count}。

    关联路径为 ErrorClusterTraces -> trace -> ObservationSpan.end_user。
    """
    if not cluster_ids:
        return {}
...
def _fetch_sessions_batch(cluster_ids: List[str]) -> dict:
    """返回 {cluster_id: distinct_session_count}。"""
    if not cluster_ids:
        return {}
...
def _fetch_latest_trace_id_batch(cluster_ids: List[str]) -> dict:
    """返回 {cluster_id: latest_trace_id_str}。

    使用单条 Postgres DISTINCT ON 查询，依赖 (cluster, -created_at) 索引，
    为每个 cluster 选出最新 membership 行，避免逐 cluster 往返查询。
    """
    if not cluster_ids:
        return {}
