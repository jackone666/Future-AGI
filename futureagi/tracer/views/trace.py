...
def _sanitize_nonfinite_floats(value):
    """递归地将 NaN/+-Infinity 浮点数替换为 ``None``。

    ClickHouse 聚合（例如对 NULL 运行 ``avgIf``、``sumIf``）以及用户传入的
    metadata/span attributes 都可能把 ``NaN``/``Infinity`` 浮点数带到响应中。
    DRF 默认的 ``json.dumps`` 会拒绝这些值并返回 500，因此序列化前需要清洗一次。
    """
...
def _build_annotation_map_from_scores(trace_ids, annotation_label_ids, label_types):
    """从 PG 的 Score 表读取 annotation 值，并构建 annotation_map。

    始终从 PG 读取，以保证写后读一致性。annotation 会先写入 PG，
    CDC 同步到 ClickHouse 可能滞后，导致新建 annotation 暂时不可见。

    Returns:
        字典结构为 trace_id -> label_id -> 结构化 annotation 数据，
        格式与 build_annotation_subqueries（PG ORM 路径）生成结果一致。
    """
...
def _build_annotation_map_from_scores_ch(trace_ids, annotation_label_ids, label_types):
    """annotation map 构建器的 ClickHouse 实现。"""
    import json
    ...
    for row in result.data:
        ...
        # 解析来自 ClickHouse 的 value JSON 字符串。
